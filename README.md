...
* **Hệ thống kiểm tra khối (Block Validation)**: Tự động phát hiện và kiểm tra tính chính xác của các khối người chơi đã xây dựng dựa trên khối mẫu (khối được click ở Điểm 1):
  * ⬜ **Viền Trắng (Đúng)**: Người chơi đã đặt khối chính xác theo mẫu.
  * 🟥 **Viền Đỏ (Sai)**: Có khối chắn tại vị trí ảo nhưng không khớp loại khối mẫu.
  * 🟨 **Viền Vàng (Thiếu)**: Vị trí ảo trống (đang là Air) cần được lấp đầy khối mẫu.
...
3. **Xem Preview và Chốt điểm cuối (Điểm 2)**:
   ...
   * Click **Chuột phải** lần nữa để khóa hình dạng cố định và kích hoạt chế độ **kiểm tra xây dựng** (White = Đúng, Red = Sai, Yellow = Thiếu).
4. **Xóa hình dạng**:
